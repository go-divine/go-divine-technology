...
let paymentUrl;
let registeredTime;
if (pageUrl.includes("=network&") == true) {
    checkOutCategory = 'network'
    checkOutItem = pageUrl.replace("?category=network&params=", "").replace(/%20/g, " ");
} else if (pageUrl.includes("=mobile&") == true) {
    checkOutCategory = 'mobile'
    checkOutItem = pageUrl.replace("?category=mobile&params=", "").replace(/%20/g, " ");
} else if (pageUrl.includes("=system&") == true) {
    checkOutCategory = 'system'
    checkOutItem = pageUrl.replace("?category=system&params=", "").replace(/%20/g, " ");
}
...
function checkOutService(service) {
    console.log(service)
  document.getElementById("titleDesktop").innerHTML  = service.title;
  document.getElementById("titleMobile").innerHTML  = service.title;
  document.getElementById("servicePricing").innerHTML = "£" + service.pricing.pricingWhole;
...
function proceedButton() {
    ...
    let postCode = document.getElementById("zipCode").value
    let titleDesktop = document.getElementById("titleDesktop").innerHTML
...
const myRequest = new Request(`https://crdquqf8.api.sanity.io/v2022-03-07/data/query/production?query=*%5B_type+%3D%3D+%22${checkOutCategory}%22%5D`);
fetch(myRequest)
