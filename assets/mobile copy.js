function displayServices({ title, pricing, serviceList, paymentUrl }) {
    ...
    <div class="service-price-card-${cardColorNumber}">
        <h3 class="price-heading">${title}</h3>
            <p class="price-text">Explore now selections our premium enterprises solution</p>
                <div class="price-values">
                    <p class="price-sign">$</p>
                <p class="actual-price">${pricing?.pricingWhole}</p>
            <div class="discount-stick-wrapper">
                <p class="discount-price">$${pricing?.pricingCent}</p>
                <div class="discount-stick-one"></div>
            </div>
        </div>
        <a href="http://localhost:5500/checkout/?params=${title?.toLowerCase()}" target="_blank" class="service-price-button-${cardColorNumber} w-button">Buy Now</a>
            ${serviceList}
    </div>
    ...
    for (let i = 0; i < serviceList.length; i++) {
        ...
        serviceGroup += `
            <div class="price-list">
                ...
                <p class="price-offer-text">${serviceList[i]}</p>
            </div>`;
...
const myRequest2 = new Request("https://crdquqf8.api.sanity.io/v2022-03-07/data/query/production?query=*%5B_type+%3D%3D+%22mobile%22%5D");
