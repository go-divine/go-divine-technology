function displayServices({ title, pricing, serviceList, paymentUrl }) {
    ...
    <div class="service-price-card-${cardColorNumber}" style="list-style: none; margin-bottom: 40px;">
        ...
        <br></br>
        ${serviceList}
        <a href="../../checkout/?category=system&params=${title?.toLowerCase()}" target="_blank" class="service-price-button-${cardColorNumber} w-button">Proceed</a>
    </div>
    `;
...
function GetServiceLists({serviceList, cardIndex}) {
    var cardColorNumber = 'price-check-icon-one';
    // if (cardIndex == 0) {
...
const myRequest2 = new Request("https://crdquqf8.api.sanity.io/v2022-03-07/data/query/production?query=*%5B_type+%3D%3D+%22system%22%5D");
